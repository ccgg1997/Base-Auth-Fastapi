  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="hidden w-60 shrink-0 flex-col border-r bg-sidebar lg:flex">
        <div className="border-b px-5 py-5"><Brand /></div>
        <nav aria-label="Navegación principal" className="flex-1 space-y-2 p-4">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
              >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between border-b bg-card px-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button aria-label="Notificaciones" className="relative hidden size-9 place-items-center rounded-lg text-muted-foreground hover:bg-muted sm:grid">
